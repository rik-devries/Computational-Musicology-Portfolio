...
main_albums_plot <- ggplot(main_album_tracks, aes(x=year_album_published, y=track.popularity, text=sprintf("Track: %s<br>Album: %s<br>Popularity: %s", track.name, track.album.name, track.popularity))) +
  labs(title="Linkin Park track popularity over time", x="Year", y="Popularity", color = "Album") +
  lims(y=c(0, 100)) +
  geom_point(aes(color=track.album.name)) + 
  geom_smooth()

ggplotly(main_albums_plot, tooltip=c("text"))
#main_albums_plot
```
